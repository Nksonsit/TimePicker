...
class CompactCalendarController {
    ...
    private int textSize = 30, badgeTextSize = 12, badgeMargin = 15;
    private int width;
    private int height;
    ...
    private int calenderBackgroundColor = Color.WHITE, badgeBackgroundColor = Color.WHITE, badgeTextColor = Color.BLACK;
    ...
    private void loadAttributes(AttributeSet attrs, Context context) {
        if (attrs != null && context != null) {
            ...
            try {
                ...
                badgeTextSize = typedArray.getDimensionPixelSize(R.styleable.CompactCalendarView_badgeTextSize,
                        (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, badgeTextSize, context.getResources().getDisplayMetrics()));

                badgeMargin = typedArray.getDimensionPixelSize(R.styleable.CompactCalendarView_badgeMargin,
                        (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, badgeMargin, context.getResources().getDisplayMetrics()));
                badgeBackgroundColor = typedArray.getColor(R.styleable.CompactCalendarView_badgeBackgroundColor, badgeBackgroundColor);
                badgeTextColor = typedArray.getColor(R.styleable.CompactCalendarView_badgeTextColor, badgeTextColor);
            } finally {
                typedArray.recycle();
            }
        }
        ...
        if (uniqEvents != null) {
            for (int i = 0; i < uniqEvents.size(); i++) {
                ...
                if (isCurrentSelectedDay || isSameDayAsCurrentDay) {
                    drawEventIndicatorCircle(canvas, xPosition, yPosition, Color.BLUE, String.valueOf(eventsList.size()));
                }
                ...
                if (!isSameDayAsCurrentDay && !isCurrentSelectedDay || animationStatus == EXPOSE_CALENDAR_ANIMATION) {
                    /*if (eventIndicatorStyle == FILL_LARGE_INDICATOR || eventIndicatorStyle == NO_FILL_LARGE_INDICATOR) {
                        ...
                        drawEventIndicatorCircle(canvas, xPosition, yPosition, event.getColor(), String.valueOf(eventsList.size()));
                    } else {*/
                    Event event = eventsList.get(0);
   ...
   /* private void drawSingleEvent(Canvas canvas, float xPosition, float yPosition, List<Event> eventsList) {
        Event event = eventsList.get(0);
        drawEventIndicatorCircle(canvas, xPosition, yPosition + yIndicatorOffset, event.getColor(), String.valueOf(eventsList.size()));
    }
    ...
    private void drawTwoEvents(Canvas canvas, float xPosition, float yPosition, List<Event> eventsList) {
        ...
        drawEventIndicatorCircle(canvas, xPosition + (xIndicatorOffset * -1), yPosition + yIndicatorOffset, eventsList.get(0).getColor(), String.valueOf(eventsList.size()));
        //draw second event just right of center
        drawEventIndicatorCircle(canvas, xPosition + (xIndicatorOffset * 1), yPosition + yIndicatorOffset, eventsList.get(1).getColor(), String.valueOf(eventsList.size()));
    }*/
    ...
    /*private void drawEventsWithPlus(Canvas canvas, float xPosition, float yPosition, List<Event> eventsList) {
        ...
        for (int j = 0, k = -2; j < 3; j++, k += 2) {
            ...
            if (j == 2) {
                ...
                drawEventIndicatorCircle(canvas, xStartPosition, yStartPosition, event.getColor(), String.valueOf(eventsList.size()));
            }
        }
    }*/
    ...
    void drawMonth(Canvas canvas, Calendar monthToDrawCalender, int offset) {
        ...
        calendar.set(Calendar.MONTH, currentCalender.get(Calendar.MONTH));
        calendar.set(Calendar.YEAR, monthToDrawCalender.get(Calendar.YEAR));
        ...
        for (int dayColumn = 0, dayRow = 0; dayColumn <= 6; dayRow++) {
            ...
            if (dayRow == 0) {
                ...
                if (day <= monthToDrawCalender.getActualMaximum(Calendar.DAY_OF_MONTH) && day > 0) {
                   ...
                   // Log.d("TAG", "drawMonth: " + String.valueOf(day));
                    ...
                    if (holidays == null || holidays.isEmpty()) {
                        ...
                        for (String holiday : holidays) {
                            ...
                            int d = Integer.parseInt(holiday.split("-")[0]);
                            int m = Integer.parseInt(holiday.split("-")[1]);
                            int y = Integer.parseInt(holiday.split("-")[2]);
                          //  Log.e("Holiday is ", String.format("%d-%d-%d", d, m, y));

                            int dc = calendar.get(Calendar.DAY_OF_MONTH);
                            int mc = calendar.get(Calendar.MONTH)+1;
                            int yc = calendar.get(Calendar.YEAR);
                        //    Log.e("Actual is ", String.format("%d-%d-%d", dc, mc, yc));
    ...
    /*private void drawDayCircleIndicator(int indicatorStyle, Canvas canvas, float x, float y, int color, String text) {
        ...
        drawCircle(canvas, x, y, color, text);
    ...
    /*private void drawCircle(Canvas canvas, float x, float y, int color, String text) {
        dayPaint.setColor(color);
        if (animationStatus == ANIMATE_INDICATORS) {
            float maxRadius = bigCircleIndicatorRadius * 1.4f;
            drawCircle(canvas, growfactorIndicator > maxRadius ? maxRadius : growfactorIndicator, x, y - (textHeight / 2), text);
        } else {
            drawCircle(canvas, bigCircleIndicatorRadius, x, y - (textHeight / 2), text);
        }
    }*/
    ...
    private void drawCircle(Canvas canvas, float x, float y, int color) {
        ...
        if (animationStatus == ANIMATE_INDICATORS) {
            float maxRadius = bigCircleIndicatorRadius * 1.4f;
            drawCircle(canvas, growfactorIndicator > maxRadius ? maxRadius : growfactorIndicator, x, y - (textHeight / 2));
    ...
    private void drawEventIndicatorCircle(Canvas canvas, float x, float y, int color, String text) {
        ...
        dayPaint.setStyle(Paint.Style.FILL);
        drawCircle(canvas, smallIndicatorRadius, x, y, text);
       /* } else if (eventIndicatorStyle == NO_FILL_LARGE_INDICATOR) {
            dayPaint.setStyle(Paint.Style.STROKE);
            drawDayCircleIndicator(NO_FILL_LARGE_INDICATOR, canvas, x, y, color, text);
        } else if (eventIndicatorStyle == FILL_LARGE_INDICATOR) {
            drawDayCircleIndicator(FILL_LARGE_INDICATOR, canvas, x, y, color, text);
        }*/
    }

    private void drawCircle(Canvas canvas, float radius, float x, float y, String text) {
        ...
        circlePaint.setColor(badgeBackgroundColor);
        circlePaint.setAntiAlias(true);

//        canvas.drawCircle(x, y - (bounds.height() / 2), bounds.width() + 5, circlePaint);
        canvas.drawCircle(x + badgeMargin, (y - badgeMargin) - (bounds.height() / 2), bounds.width() + 5, circlePaint);
//        canvas.drawCircle(x, y, radius, circlePaint);

        canvas.drawText(text, x + badgeMargin, y - badgeMargin, paint);
    }
